...
2. Open a blank ESP32 simulator canvas on Wokwi.
...
```text
docs: establish core project architecture and document system requirements in README
```
...
```text
hw: design and verify virtual ESP32 sensor canvas layout schema
```
...
```text
firmware: implement non-blocking sensor acquisition loops and ADC normalization scaling
```
...
```text
firmware: embed closed-loop irrigation automation and water tank safety lockout
```
...
```text
net: integrate Wi-Fi stack and configure telemetry payload streaming to ThingSpeak cloud
```
...
```text
analytics: deploy local python data logger pipeline and complete validation documentation
```
